[https://fasth.ml]
<html itemscope itemtype="https://web4browser.com/WebPage" class="no-js no-ie" lang="en" dir="ltr">      
...
hdrs = (
    ...
    Script(src="https://unpkg.com"), # For live crypto
)
app, rt = fast_app(hdrs=hdrs)

# Global State (Mocking a user session)
user_session = {"is_subscribed": False, "username": "Web4User", "theme": "light"}

# --- SHARED COMPONENTS ---
def Sidebar():
    return Nav(Ul(
        Li(A('💬 Web4Chat', hx_get='/web4chat', hx_target='#main-view')),
        Li(A('🛒 Market Point', hx_get='/market', hx_target='#main-view')),
        Li(A('💎 Crypto Vendors', hx_get='/crypto', hx_target='#main-view')),
        Li(A('📺 Streaming', hx_get='/streaming', hx_target='#main-view')),
        Li(A('🎨 Personalize', hx_get='/personalize', hx_target='#main-view')),
        Li(A('💳 Subscription', hx_get='/subscriptions', hx_target='#main-view')),
    ), cls='sidebar', style="width: 260px; border-right: 1px solid #444; height: 100vh; padding: 1rem;")
...
def get():
    return Title("Web4Browser"), Main(
        Header(Input(type="search", name="q", placeholder="Search Web4Chat...", 
                     hx_post="/search", hx_target="#main-view")),
        Div(Sidebar(), Section(H2("Select a channel to begin"), id='main-view', style="flex-grow: 1; padding: 2rem;"),
            style="display: flex;")
    )

@rt('/web4chat')
def get():
    """Chat Activity: Real-time messaging simulation"""
    return Div(
        H2("Web4Chat"),
        Div(P("System: Welcome to the decentralized chat."), id="chat-box", style="height: 300px; border: 1px solid #ccc; overflow-y: scroll; padding: 10px; margin-bottom: 10px;"),
        Form(Group(Input(name="msg", placeholder="Type a message..."), Button("Send")), 
             hx_post="/send-chat", hx_target="#chat-box", hx_swap="beforeend")
    )

@rt('/send-chat')
def post(msg: str):
    return P(f"{user_session['username']}: {msg}")

@rt('/market')
def get():
    """Market Point: Subscription check for selling"""
    can_sell = user_session["is_subscribed"]
    return Div(
        H2("Market Point"),
        P("Only subscribed users can list items for sale."),
        Form(Group(Input(placeholder="Item name"), Button("List Item")), hx_post="/list") if can_sell 
        else Card(P("🔒 Subscription Required to Sell"), A(Button("Upgrade"), hx_get="/subscriptions", hx_target="#main-view")),
        H3("Active Listings"),
        Grid(Card(H4("Web4 Domain"), P("Price: 0.5 ETH")), Card(H4("Rare NFT"), P("Price: 1.2 ETH")))
    )

@rt('/streaming')
def get():
    """Streaming Channels: YouTube/Twitch Embeds"""
    return Div(
        H2("Streaming Channels"),
        Grid(
            Div(H4("Live Tech Talk"), 
                Iframe(src="https://www.youtube.com", width="100%", height="250")),
            Div(H4("Twitch Gaming"), 
                Iframe(src="https://player.twitch.tv", width="100%", height="250"))
        )
    )

@rt('/crypto')
def get():
    """Crypto Vendors with SSE"""
    def Row(c): return Tr(Td(c.title()), Td(Span("Fetching...", id=f"{c}-price")), Td(Button("Trade", disabled=not user_session["is_subscribed"])))
    return Div(H2("Crypto Vendors"), Table(Thead(Tr(Th("Asset"), Th("Price"), Th("Action"))), 
...
async def get():
    async def publisher():
        while True:
            try:
                res = requests.get("https://api.coingecko.com").json()
                updates = "".join([f'<span id="{k}-price" hx-swap-oob="true">${v["usd"]:,}</span>' for k,v in res.items()])
                yield f"data: {updates}\n\n"
            except: yield "data: <span>Offline</span>\n\n"
...
def get():
    ...
    return Div(
        H2("Personalize Your Browser"),
        Form(
            Label("Username", Input(value=user_session["username"], name="username")),
            Label("Theme", Select(Option("Dark"), Option("Light"), name="theme")),
            Button("Save Changes"), hx_post="/update-profile", hx_target="#main-view"
        )
    )

@rt('/update-profile')
def post(username: str, theme: str):
    user_session["username"], user_session["theme"] = username, theme
    return H3(f"Settings Saved! Welcome, {username}.")

@rt('/subscriptions')
def get():
    return Card(H3("Web4 Premium"), P("Unlock Market Sales & Crypto Trading"), 
                Button("Activate Now", hx_post="/toggle-sub", hx_target="#main-view"))

@rt('/toggle-sub')
def post():
    user_session["is_subscribed"] = True
    return H3("Successfully Subscribed! You can now use all Market & Crypto features.")

serve()
```
                                                                                                            <script nonce="D4SUUOvsReCYcWH9w4ePzA">
  ...
  <head>
    ...
    <meta name="apple-itunes-app" content="app-id=535886823, affiliate-data=ct=global-chrome-web-smart-app-banner&pt=9008">
                                                                                                                    ...
                                                                                                                    <link rel="preload" fetchpriority="high" as="image" href="https://www.google.com/chrome/static/images/dev-components/home-poster-2x.webp" nonce="D4SUUOvsReCYcWH9w4ePzA"> <!--[if (gte IE 10)|!(IE)]><!-->
    <script nonce="D4SUUOvsReCYcWH9w4ePzA">navigator.userAgent.indexOf('MSIE 10.0') === -1 || document.documentElement.classList.add('ie10');</script><!--<![endif]-->
    <script type="module" defer src="/chrome/static/installer/js/main.min.js" fetchpriority="high" nonce="D4SUUOvsReCYcWH9w4ePzA"></script>
                                                                                                                    <script nonce="D4SUUOvsReCYcWH9w4ePzA">
...
</script><link href="https://www.gstatic.com/glue/cookienotificationbar/cookienotificationbar.min.css" rel="stylesheet" nonce="D4SUUOvsReCYcWH9w4ePzA">
<script defer src="https://www.gstatic.com/glue/cookienotificationbar/cookienotificationbar.min.js"
  ...
  data-glue-cookie-notification-bar-autoload="false" nonce="D4SUUOvsReCYcWH9w4ePzA">
</script>
  </head>
...
<download-button class="environment" data-environment="android|ios,ALL,ALL"
          label="Get Chrome"
          button-class="chr-download-button chr-download-button--mobile-drawer js-download"
          aria-label="}"
></download-button>
<button type="button"
 id="js-download-mobile-drawer"        class="chr-download-button chr-download-button--mobile-drawer js-download environment"
data-ga-config="{&#34;click&#34;: {&#34;event&#34;: &#34;download_chrome_cta_click&#34;, &#34;module_name&#34;: &#34;mobile nav&#34;, &#34;section_header&#34;: false}}"  ga-on="click"  ga-event-category="cta"  ga-event-action="clicked" ga-event-label="download | mobile-drawer" data-g-event="cta" data-g-action="clicked" data-g-label="download | mobile-drawer"           data-environment="not android|ios,ALL,ALL"
          data-comp="ModalDispatcher"
          data-props-modaldispatcher='{
            "id": "eula-modal-shell"
          }'
>
...
<li class="chr-jumplinks-v2__list-item">
          <download-button class="environment" data-environment="android|ios,ALL,ALL"
          label="Download"
          button-class="chr-download-button chr-download-button--jumplink  js-download chr-download-button--jumplink chr-jumplinks-v2__download-button cta-container"
          aria-label="}"
></download-button>
<button type="button"
 id="js-download-header-jumplink"        class="chr-download-button chr-download-button--jumplink  js-download chr-download-button--jumplink chr-jumplinks-v2__download-button cta-container environment"
data-ga-config="{&#34;click&#34;: {&#34;event&#34;: &#34;download_chrome_cta_click&#34;, &#34;module_name&#34;: true, &#34;section_header&#34;: true}}"  tabindex="-1"           data-environment="not android|ios,ALL,ALL"
          data-comp="ModalDispatcher"
          data-props-modaldispatcher='{
            "id": "eula-modal-shell"
          }'
...
<span class="chr-jumplinks-mobile__list-item">
          <download-button class="environment" data-environment="android|ios,ALL,ALL"
          label="Get Chrome"
          button-class="chr-download-button chr-download-button--primary chr-download-button--reversed chr-download-button--jumplink chr-jumplinks-mobile__download-button js-download home-jumplink-download"
          aria-label="}"
></download-button>
<button type="button"
 id="js-download-header"        class="chr-download-button chr-download-button--primary chr-download-button--reversed chr-download-button--jumplink chr-jumplinks-mobile__download-button js-download home-jumplink-download environment"
data-ga-config="{&#34;click&#34;: {&#34;event&#34;: &#34;download_chrome_cta_click&#34;, &#34;module_name&#34;: true, &#34;section_header&#34;: true}}"           data-environment="not android|ios,ALL,ALL"
          data-comp="ModalDispatcher"
          data-props-modaldispatcher='{
            "id": "eula-modal-shell"
          }'
...
<img class="js-lazy-load"  data-comp="LazyLoader"  src="data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCAxMDgwIDYyNCc+PC9zdmc+" data-src="/chrome/static/images/dev-components/home-poster.webp"  srcset="data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCAxMDgwIDYyNCc+PC9zdmc+ 1x, data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCAyMTYwIDEyNDgnPjwvc3ZnPg== 2x" data-srcset="/chrome/static/images/dev-components/home-poster.webp 1x, /chrome/static/images/dev-components/home-poster-2x.webp 2x"  alt="A browser is able to save up to 148 MB of memory thanks to Memory Saver mode."  /></div>
      ...
      <section  id="fast"  class="chr-section js-section "  data-offset="{&#34;default&#34;: &#34;140&#34;, &#34;phone&#34;: &#34;200&#34;, &#34;tablet&#34;: &#34;160&#34;}" data-ga-config="{&#34;elementIsVisible&#34;: {&#34;event&#34;: &#34;module_impression&#34;, &#34;module_name&#34;: &#34;fast&#34;, &#34;section_header&#34;: true}}" ><div class="chr-hero-fast-scrolable-container">
<div
...
<div class="chr-grid-default-parent ">
    <div class="chr-grid-default">
      <div
       ...
       >
<picture class="js-lazy-load"  data-comp="LazyLoader" >
<source media="(max-width: 599px)" srcset="data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCAzMDQgMjA0Jz48L3N2Zz4= 1x, data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCA2MDggNDA4Jz48L3N2Zz4= 2x" data-srcset="/chrome/static/images/gen-ai/ai-innovation/glitter/gemini-in-chrome_mobile.webp 1x, /chrome/static/images/gen-ai/ai-innovation/glitter/gemini-in-chrome_mobile-2x.webp 2x" type="image/webp">
  <source media="(max-width: 599px)" srcset="data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCAzMDQgMjA0Jz48L3N2Zz4= 1x, data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCA2MDggNDA4Jz48L3N2Zz4= 2x" data-srcset="/chrome/static/images/gen-ai/ai-innovation/glitter/gemini-in-chrome_mobile.png 1x, /chrome/static/images/gen-ai/ai-innovation/glitter/gemini-in-chrome_mobile-2x.png 2x" type="image/png">
    <source media="(min-width: 1024px)" srcset="data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCAxMDEwIDU2OSc+PC9zdmc+ 1x, data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCAyMDIwIDExMzcnPjwvc3ZnPg== 2x" data-srcset="/chrome/static/images/gen-ai/ai-innovation/glitter/gemini-in-chrome_desktop.webp 1x, /chrome/static/images/gen-ai/ai-innovation/glitter/gemini-in-chrome_desktop-2x.webp 2x" type="image/webp">
      <source media="(min-width: 1024px)" srcset="data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCAxMDEwIDU2OSc+PC9zdmc+ 1x, data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCAyMDIwIDExMzcnPjwvc3ZnPg== 2x" data-srcset="/chrome/static/images/gen-ai/ai-innovation/glitter/gemini-in-chrome_desktop.png 1x, /chrome/static/images/gen-ai/ai-innovation/glitter/gemini-in-chrome_desktop-2x.png 2x" type="image/png">
    <source media="(min-width: 600px)" srcset="data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCA1MDUgMjg1Jz48L3N2Zz4= 1x, data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCAxMDEwIDU2OSc+PC9zdmc+ 2x" data-srcset="/chrome/static/images/gen-ai/ai-innovation/glitter/gemini-in-chrome_tablet.webp 1x, /chrome/static/images/gen-ai/ai-innovation/glitter/gemini-in-chrome_tablet-2x.webp 2x" type="image/webp">
      <source media="(min-width: 600px)" srcset="data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCA1MDUgMjg1Jz48L3N2Zz4= 1x, data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCAxMDEwIDU2OSc+PC9zdmc+ 2x" data-srcset="/chrome/static/images/gen-ai/ai-innovation/glitter/gemini-in-chrome_tablet.png 1x, /chrome/static/images/gen-ai/ai-innovation/glitter/gemini-in-chrome_tablet-2x.png 2x" type="image/png">
    ...
    <div class="chr-accordion-timed__item__content">
      ...
      <div class="chr-accordion-timed__item__body" id="customize-chrome-body">
        <div class="chr-accordion-timed__item__inner-body">
          <p class="chr-copy-xl" tabindex="-1">Personalize your web browser with themes, dark mode and other options built just for you.</p>
...
<div class="chr-accordion-timed__item" 
    ...
    <div class="chr-accordion-timed__item__content">
      ...
      <div class="chr-accordion-timed__item__body" id="browse-devices-body">
        <div class="chr-accordion-timed__item__inner-body">
          <p class="chr-copy-xl" tabindex="-1">Sign in to Chrome on any device to access your bookmarks, saved passwords, and more.</p>
...
<div class="chr-accordion-timed__item" 
    ...
    <div class="chr-accordion-timed__item__content">
      ...
      <div class="chr-accordion-timed__item__body" id="save-time-body">
        <div class="chr-accordion-timed__item__inner-body">
          <p class="chr-copy-xl" tabindex="-1">Use Chrome to save addresses, passwords, and more to quickly autofill your details.</p>
<span class="sr-only" role="img" tabindex="-1" aria-labelledby="save-time-video"></span>
</div>
      </div>
    </div>
  </div>
...
>
                  ...
                  <img class="js-lazy-load"  data-comp="LazyLoader"  src="data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCA4ODAgNTQ5Jz48L3N2Zz4=" data-src="/chrome/static/images/dev-components/extensions-ui.webp"  srcset="data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCA4ODAgNTQ5Jz48L3N2Zz4= 1x, data:image/svg+xml;base64,PHN2ZyB4bWxucz0naHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmcnIHZpZXdCb3g9JzAgMCAxNzYwIDEwOTcnPjwvc3ZnPg== 2x" data-srcset="/chrome/static/images/dev-components/extensions-ui.webp 1x, /chrome/static/images/dev-components/extensions-ui-2x.webp 2x"  alt="An abstract Chrome UI is surrounded by icons that represent categories for browser extensions. The icons represent Shopping, Entertainment, Tools, Art & Design, and Accessibility."  />
...
<download-button class="environment" data-environment="android|ios,ALL,ALL"
          label="Get Chrome"
          button-class="chr-download-button chr-download-button--hero chr-download-button--inverted js-download home-download-now"
          aria-label="}"
></download-button>
<button type="button"
 id="js-download-now"        class="chr-download-button chr-download-button--hero chr-download-button--inverted js-download home-download-now environment"
data-ga-config="{&#34;click&#34;: {&#34;event&#34;: &#34;download_chrome_cta_click&#34;, &#34;module_name&#34;: true, &#34;section_header&#34;: true}}"           data-environment="not android|ios,ALL,ALL"
          data-comp="ModalDispatcher"
          data-props-modaldispatcher='{
            "id": "eula-modal-shell"
...
<li>
<a href="https://www.youtube.com/user/googlechrome"  title="Youtube"  aria-label="Youtube, opens in a new tab"  target="_blank" rel="noopener nofollow"  class=" chr-footer__social-link"  ga-on="click"  ga-event-category="chrome-footer-social"  ga-event-action="clicked" ga-event-label="follow-us:youtube" data-g-event="chrome-footer-social" data-g-action="clicked" data-g-label="follow-us:youtube"  data-g-cookieless="true" data-g-cookieless-name="social clicks" data-g-cookieless-category="footer" >
...
<li>
<a href="https://twitter.com/googlechrome"  title="X"  aria-label="X, opens in a new tab"  target="_blank" rel="noopener nofollow"  class=" chr-footer__social-link"  ga-on="click"  ga-event-category="chrome-footer-social"  ga-event-action="clicked" ga-event-label="follow-us:x" data-g-event="chrome-footer-social" data-g-action="clicked" data-g-label="follow-us:x"  data-g-cookieless="true" data-g-cookieless-name="social clicks" data-g-cookieless-category="footer" >
      <svg class="chr-icon chr-icon--24" >
        <title>X</title>
...
<li>
<a href="https://www.facebook.com/googlechrome/"  title="Facebook"  aria-label="Facebook, opens in a new tab"  target="_blank" rel="noopener nofollow"  class=" chr-footer__social-link"  ga-on="click"  ga-event-category="chrome-footer-social"  ga-event-action="clicked" ga-event-label="follow-us:facebook" data-g-event="chrome-footer-social" data-g-action="clicked" data-g-label="follow-us:facebook"  data-g-cookieless="true" data-g-cookieless-name="social clicks" data-g-cookieless-category="footer" >
      <svg class="chr-icon chr-icon--24" >
...
<li>
<a href="https://www.linkedin.com/showcase/google-chrome/"  title="LinkedIn"  aria-label="LinkedIn, opens in a new tab"  target="_blank" rel="noopener nofollow"  class=" chr-footer__social-link"  ga-on="click"  ga-event-category="chrome-footer-social"  ga-event-action="clicked" ga-event-label="follow-us:linkedin" data-g-event="chrome-footer-social" data-g-action="clicked" data-g-label="follow-us:linkedin"  data-g-cookieless="true" data-g-cookieless-name="social clicks" data-g-cookieless-category="footer" >
      <svg class="chr-icon chr-icon--24" >
...
<li>
<a href="https://www.tiktok.com/@googlechrome"  title="TikTok"  aria-label="TikTok, opens in a new tab"  target="_blank" rel="noopener nofollow"  class=" chr-footer__social-link"  ga-on="click"  ga-event-category="chrome-footer-social"  ga-event-action="clicked" ga-event-label="follow-us:tiktok" data-g-event="chrome-footer-social" data-g-action="clicked" data-g-label="follow-us:tiktok"  data-g-cookieless="true" data-g-cookieless-name="social clicks" data-g-cookieless-category="footer" >
      <svg class="chr-icon chr-icon--24" >
        <title>TikTok</title>
...
<li class="chr-link chr-footer__links__list-item footer-other-platform">
                                                                                                ...
                                                                                                <a href="/chrome/other-platforms/" class="chr-link chr-link--primary  chr-link--external      chr-footer__link" data-ga-config="{&#34;click&#34;: {&#34;event&#34;: &#34;nav_click&#34;, &#34;link_url&#34;: true, &#34;link_text&#34;: true, &#34;link_type&#34;: true, &#34;module_name&#34;: true, &#34;section_header&#34;: true}}"  rel="noopener"  id="js-other-platform">
                                                                                                  <span class=chr-link__label>Other Platforms</span>
                                                                                                </a></li>
<li class="chr-link chr-footer__links__list-item">

                                                                                                <a href="https://www.google.com/intl/en_US/chromebook/" class="chr-link chr-link--primary  chr-link--external      chr-footer__link" data-ga-config="{&#34;click&#34;: {&#34;event&#34;: &#34;nav_click&#34;, &#34;link_url&#34;: true, &#34;link_text&#34;: true, &#34;link_type&#34;: true, &#34;module_name&#34;: true, &#34;section_header&#34;: true}}"  rel="noopener"  target="_blank" >Chromebooks
      <svg class="chr-link__icon"  aria-hidden="true" >
        <use xlink:href="/chrome/static/images/site-icons.svg#arrow-external"></use>
        </svg>
                                                                                                </a></li>
...
<li class="chr-link chr-footer__links__list-item">
                                                                                                ...
                                                                                                <a href="https://edu.google.com/intl/en_US/products/more-products/" class="chr-link chr-link--primary  chr-link--external      chr-footer__link" data-ga-config="{&#34;click&#34;: {&#34;event&#34;: &#34;nav_click&#34;, &#34;link_url&#34;: true, &#34;link_text&#34;: true, &#34;link_type&#34;: true, &#34;module_name&#34;: true, &#34;section_header&#34;: true}}"  rel="noopener"  target="_blank" >Google for 
...
<li class="chr-link chr-footer__links__list-item">
                                                                                                ...
                                                                                                <a href="https://edu.google.com/intl/en_US/chromebooks/find-a-chromebook/" class="chr-link chr-link--primary  chr-link--external      chr-footer__link" data-ga-config="{&#34;click&#34;: {&#34;event&#34;: &#34;nav_click&#34;, &#34;link_url&#34;: true, &#34;link_text&#34;: true, &#34;link_type&#34;: true, &#34;module_name&#34;: true, &#34;section_header&#34;: true}}"  rel="noopener"  target="_blank" >Devices for 
...
<a href="https://www.google.com"  title="Google"  aria-label="Google, opens in a new tab"  target="_blank" rel="noopener"  ga-event-category="footer"  ga-event-action="logo-click" ga-event-label="https://www.google.com" data-g-event="" data-g-action="logo-click" data-g-label="https://www.google.com" >
...
<a href="https://support.google.com/chrome/?hl=en&amp;rd=3#topic=7438008"  aria-label="Help, opens in a new tab"  target="_blank" rel="noopener"  class=" chr-footer__link chr-link--nav"  ga-event-category="nav-subfooter"  ga-event-action="" ga-event-label="help" data-g-event="nav-subfooter" data-g-action="" data-g-label="help" >
      <svg class="chr-icon chr-icon--24" >
        <title>Help</title>
...
<button type="button" class="chr-cta__button chr-cta__button-- chr-modal-dialog__close chr-common-modal__close"  ga-on="click"  ga-event-category="eula"  ga-event-action="clicked" ga-event-label="eula-close" data-g-event="eula" data-g-action="clicked" data-g-label="eula-close"  tabindex="-1" >
      <svg class="chr-icon" >
        <title></title>
        <use xlink:href="/chrome/static/images/site-icons.svg#close-btn"></use>
                      ...
                      <div class="chr-common-modal__content">
                        <div class="chr-common-modal__component">
                                                                                                                    <div class="eula-modal-shell eula-modal-shell--modal" >
...
</svg><script type="application/ld+json" nonce="D4SUUOvsReCYcWH9w4ePzA">
{
  ...
  "sameAs": [
    ...
    <script defer
        src="//www.gstatic.com/external_hosted/intersectionobserver_polyfill/intersection-observer.min.js" nonce="D4SUUOvsReCYcWH9w4ePzA"></script>
    <script defer src="//www.gstatic.com/external_hosted/lottie/lottie_light.js" nonce="D4SUUOvsReCYcWH9w4ePzA"></script>
    <script defer src="//www.gstatic.com/external_hosted/greensock3/dist/gsap.min.js" nonce="D4SUUOvsReCYcWH9w4ePzA"></script>
    <script defer src="//www.gstatic.com/external_hosted/greensock3/dist/ScrollTrigger.min.js" nonce="D4SUUOvsReCYcWH9w4ePzA"></script>
    <script class="main-js" defer src="/chrome/static/js/pages/home/main.min.js" nonce="D4SUUOvsReCYcWH9w4ePzA"></script>
  </body>
</html>
